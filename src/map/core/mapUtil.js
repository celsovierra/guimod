...
export const prepareIcon = (background, icon, color) => {
  ...
  // If a background is provided (used for cluster/background icons), keep existing behaviour
  if (background) {
    ...
    // No background: create canvas matching icon size so the image has transparent background
    ...
    // If a color is provided, tint the icon as before. If not, draw the icon as-is
    // to preserve original PNG transparency and colors (useful for top-view PNGs).
    if (color) {
      const tinted = canvasTintImage(icon, color);
      context.drawImage(tinted, 0, 0, canvas.width, canvas.height);
    } else {
      context.drawImage(icon, 0, 0, canvas.width, canvas.height);
    }
  } else {
    // Fallback small transparent canvas
    canvas.width = 16 * devicePixelRatio;
